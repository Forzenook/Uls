...
static void mx_print_dir_rec(t_entry *directory, bool print_dir_name, bool print_newline_in_the_beginning) {
    ...
    if (directory->no_permission) {
        ...
        size_t name_index_last = mx_strlen(path) - 1;
        for ( ; path[name_index_last] == '/' ; name_index_last--) { }
        int name_index_first = name_index_last;
        for ( ; path[name_index_first] != '/' && name_index_first != -1; name_index_first--) { }
        name_index_first++;

        mx_printerr("uls");
        mx_printerr(": ");
        mx_printerrn(path + name_index_first, name_index_last - name_index_first + 1);
        mx_printerr(": ");
